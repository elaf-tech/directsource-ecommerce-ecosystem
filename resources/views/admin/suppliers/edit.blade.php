...
<style>
    ...
    /* --- قسم الشعار --- */
    .logo-upload-section { display: flex; align-items: center; gap: 20px; }
    .current-logo-preview { width: 80px; height: 80px; border-radius: 50%; object-fit: cover; border: 3px solid var(--primary-color); }
...
<div class="page-header">
    <h2 class="page-title"><i class="fas fa-edit"></i> تعديل بيانات المورد</h2>
    <a href="{{ route('supplierad.index') }}" class="btn-back"><i class="fas fa-arrow-right"></i> العودة لقائمة الموردين</a>
</div>

<div class="form-card">
    <form action="{{ route('supplierad.update', $supplier->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')

        <div class="form-header">
            <i class="fas fa-truck-loading"></i>
            <h3>تعديل بيانات: {{ $supplier->company_name }}</h3>
        </div>
        ...
        <div class="form-body">
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" name="company_name" value="{{ old('company_name', $supplier->company_name) }}" required placeholder="اسم الشركة">
                        @error('company_name') <small class="text-danger">{{ $message }}</small> @enderror
                    </div>
                    <div class="form-group">
                        <i class="fas fa-briefcase form-icon"></i>
                        <input class="form-control" type="text" name="business_type" value="{{ old('business_type', $supplier->business_type) }}" required placeholder="نوع النشاط">
                        @error('business_type') <small class="text-danger">{{ $message }}</small> @enderror
                    </div>
                </div>
                <div class="form-group mt-3">
                    <i class="fas fa-map-marker-alt form-icon"></i>
                    <input class="form-control" type="text" name="address" value="{{ old('address', $supplier->address) }}" required placeholder="العنوان التفصيلي">
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" name="identity_number" value="{{ old('identity_number', $supplier->identity_number) }}" required placeholder="رقم الهوية / الإقامة">
                        @error('identity_number') <small class="text-danger">{{ $message }}</small> @enderror
                    </div>
                    <div class="form-group">
                        <i class="fas fa-file-contract form-icon"></i>
                        <input class="form-control" type="text" name="commercial_registration_number" value="{{ old('commercial_registration_number', $supplier->commercial_registration_number) }}" required placeholder="رقم السجل التجاري">
                        @error('commercial_registration_number') <small class="text-danger">{{ $message }}</small> @enderror
                    </div>
                </div>
                <div class="logo-upload-section mt-3">
                    <img src="{{ $supplier->logo ? asset('storage/'.$supplier->logo) : 'https://ui-avatars.com/api/?name=L&background=8aa57c&color=fff' }}" alt="الشعار الحالي" class="current-logo-preview">
            ...
            <div class="form-section">
                <h4 class="form-section-title">المعلومات المالية</h4>
                <div class="form-group">
                    <i class="fas fa-credit-card form-icon"></i>
                    <input class="form-control" type="text" name="bank_account" value="{{ old('bank_account', $supplier->bank_account) }}" required placeholder="رقم الحساب البنكي (IBAN)">
