...
<div class="page-header">
    <h2 class="page-title"><i class="fas fa-sitemap"></i> إدارة الفئات</h2>
    <a href="{{ route('categoryad.create') }}" class="btn-add-category">
...
<div class="categories-grid">
    @forelse($cat as $category)
        <div class="category-card">
            <div class="category-image-wrapper">
                ...
                @else
                    <img src="https://via.placeholder.com/300x200.png/f0f5eb/4e6b3a?text={{ urlencode($category->name ) }}" class="category-image" alt="{{ $category->name }}">
                @endif
            </div>
            <div class="card-content">
                <h5 class="category-title">{{ $category->name }}</h5>
                <p class="category-desc">
                    {{ Str::limit($category->description ?? 'لا يوجد وصف لهذه الفئة.', 90) }}
                </p>
            </div>
            
            <!-- **الجديد: تذييل البطاقة مع أزرار الإجراءات** -->
            <div class="card-footer">
                <a href="{{ route('categoryad.edit', $category->id) }}" class="btn-action btn-edit">
                    <i class="fas fa-pen"></i> تعديل
                </a>
                <form action="{{ route('categoryad.destroy', $category->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذه الفئة؟');">
                    @csrf
                    @method('DELETE')
    ...
    @empty
        <div class="no-categories-message">
            ...
            <p>يمكنك إضافة فئة جديدة من الزر في الأعلى.</p>
        </div>
    @endforelse
</div>
